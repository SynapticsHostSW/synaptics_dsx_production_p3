		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		dev_err(&rmidev->rmi4_data->pdev->dev,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_dbg(&rmidev->rmi4_data->pdev->dev,
		...
		struct device_attribute *attr, char *buf)
{
	...
	const struct synaptics_dsx_platform_data *platform_data =
			rmidev->rmi4_data->hw_info->board;
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmidev->fn_ptr->read = rmi4_data->hw_info->bus->read;
	rmidev->fn_ptr->write = rmi4_data->hw_info->bus->write;
	...
	retval = gpio_export(rmi4_data->hw_info->board->irq_gpio, false);
	if (retval < 0) {
		dev_err(&rmi4_data->pdev->dev,
		...
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", rmi4_data->hw_info->board->irq_gpio);
		if (retval < 0) {
			...
			dev_dbg(&rmi4_data->input_dev->dev,
				...
				__func__, rmi4_data->hw_info->board->irq_gpio);
		}
	}
