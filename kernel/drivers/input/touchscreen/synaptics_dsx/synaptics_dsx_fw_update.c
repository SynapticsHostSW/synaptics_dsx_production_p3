...
static int fwu_read_f01_device_status(struct f01_device_status *status)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			sizeof(status->data));
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->pdev->dev,
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	dev_info(&fwu->rmi4_data->pdev->dev,
...
static int fwu_scan_pdt(void)
{
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		...
		if (rmi_fd.fn_number) {
			dev_dbg(&fwu->rmi4_data->pdev->dev,
					"%s: Found F%02x\n",
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	fwu->fn_ptr->read = rmi4_data->hw_info->bus->read;
	fwu->fn_ptr->write = rmi4_data->hw_info->bus->write;
